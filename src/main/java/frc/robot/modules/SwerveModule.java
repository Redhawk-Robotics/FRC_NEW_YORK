...
public class SwerveModule {
     ...
     * @param lastAngle              In degrees, the last angle that was inputted
     *                               within the code
     * 
    ...
    // Creating a FeedForward setting for a reference tracking for the desired
    // output of the motor
    SimpleMotorFeedforward feedForward = new SimpleMotorFeedforward(Settings.driveKS, Settings.driveKV,
            Settings.driveKA);// FIXME find values
     ...
     */
    // Creates desiredState of the Swervemodules which is WPILIB solution for
    ...
    private void configAngleMotor() {
        ...
        angleMotor.setSmartCurrentLimit(Settings.angleContinousCurentLimit);
        angleMotor.setInverted(Settings.angleMotorInvert);
        angleMotor.setIdleMode(Settings.angleNeutralMode);
        integratedAngleEncoder.setPositionConversionFactor(Settings.angleConversionFactor);
        angleController.setP(Settings.angleKP);
        angleController.setI(Settings.angleKI);
        angleController.setD(Settings.angleKD);
        angleController.setFF(Settings.angleKFF);
        angleMotor.enableVoltageCompensation(Settings.voltageComp);
    ...
    private void configDriveMotor() {
        ...
        driveMotor.setSmartCurrentLimit(Settings.driveContinousCurrentLimit);
        driveMotor.setIdleMode(Settings.driveNeutralMode);
        driveEncoder.setVelocityConversionFactor(Settings.driveConversionVelocityFactor);
        driveEncoder.setPositionConversionFactor(Settings.driveConversionPositionFactor);
        driveController.setP(Settings.driveKP);
        driveController.setI(Settings.driveKI);
        driveController.setD(Settings.driveKD);
        driveController.setFF(Settings.driveKFF);
        driveMotor.enableVoltageCompensation(Settings.voltageComp);
    ...
    private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        if (isOpenLoop) {
            double percentOutput = desiredState.speedMetersPerSecond / Settings.maxVelocityMetersPerSecond;
    ...
    private void setAngle(SwerveModuleState desiredState) {
        ...
        Rotation2d angle = (Math.abs(desiredState.speedMetersPerSecond) <= (Settings.maxVelocityMetersPerSecond * 0.01))
                ? lastAngle
                : desiredState.angle;
    ...
    // Method in order to get the States of the SwerveDrive module of the velocity
    // and angle of the Drive Motor
    public SwerveModuleState getState() {
        return new SwerveModuleState(driveEncoder.getVelocity(), getAngle());
    }

    // SmartDashBoard information of the Encoder values of the Integrated and
    // CanCoder of Angle Motor
